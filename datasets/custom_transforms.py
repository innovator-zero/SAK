...
def get_transformations(dataname, size, train=True):
    """
    ...
    """
    if dataname == "pascalcontext" or dataname == "nyud":
        if train:
...
class RandomCrop(object):
    ...
    def get_random_crop_loc(self, uncropped):
        ...
        crop_loc = [
            offset_height,
            offset_height + desired_height,
            offset_width,
            offset_width + desired_width,
        ]
...
class PadImage(object):
    ...
    def pad(self, key, unpadded):
        ...
        padded = np.full(
            (max_height, max_width, unpadded_shape[2]), pad_value, dtype=unpadded.dtype
        )
        padded[
            height_location[0] : height_location[1],
...
class PhotoMetricDistortion:
    ...
    def hue(self, img):
        ...
        if random.random() < 0.5:
            img[:, :, 0] = (
                img[:, :, 0].astype(int)
                + random.randint(-self.hue_delta, self.hue_delta - 1)
            ) % 180
        return img
