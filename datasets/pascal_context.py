...
class PASCALContext(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        root=MyPath.db_root_dir("PASCALContext"),
        train=True,
        tasks=None,
        transform=None,
        dataidxs=None,
        ...
        self.cat_part = json.load(
            open(os.path.join(self.root, "json/pascal_part.json"), "r")
        )
        ...
        if self.do_human_parts:
            if not self._check_preprocess_parts():
                print(
                    "Preprocessing PASCAL dataset for human parts, this will take long, but will be done only once."
                )
        ...
        if self.do_human_parts:
            ...
            if (
                not self.do_edge
                and not self.do_semseg
                and not self.do_sal
                and not self.do_normals
            ):
    ...
    def __build_truncated_dataset__(self):
        if self.dataidxs is not None:
            ...
            self.semsegs = [
                self.semsegs[idx] for idx in self.dataidxs if self.do_semseg
            ]
            self.parts = [
                self.parts[idx] for idx in self.dataidxs if self.do_human_parts
            ]
            self.sals = [self.sals[idx] for idx in self.dataidxs if self.do_sal]
            self.normals = [
                self.normals[idx] for idx in self.dataidxs if self.do_normals
            ]
    ...
    def __getitem__(self, index):
        ...
        sample["meta"] = {
            "file_name": str(self.im_ids[index]),
    ...
    def _load_img(self, index):
        _img = np.array(
            Image.open(self.images[index]).convert("RGB"), dtype=np.float32, copy=False
        )
        return _img
    ...
    def _load_edge(self, index):
        ...
        _edge = cv2.Laplacian(
            _tmp["LabelMap"], cv2.CV_64F
        )  # Laplacian, detect edges of different context labels
    ...
    def _load_semseg(self, index):
        _semseg = np.array(
            Image.open(self.semsegs[index]), dtype=np.float32, copy=False
        )
        return _semseg
            ...
            for _obj_ii in range(len(_part_mat)):
                ...
                if has_human and has_parts:
                    if _inst_mask is None:  # The first human in the image
                        _inst_mask = _part_mat[_obj_ii][2].astype(
                            np.float32
                        )  # Mask of positions contains human
                    ...
                    for part_i in range(n_parts):
                        cat_part = str(
                            _part_mat[_obj_ii][3][0][part_i][0][0]
                        )  # Name of part
                        mask_id = self.cat_part[str(self.human_parts_category)][
                            cat_part
                        ]
                        mask = _part_mat[_obj_ii][3][0][part_i][1].astype(
                            bool
                        )  # Position of part
    ...
    def _load_sal(self, index):
        _sal = (
            np.array(Image.open(self.sals[index]), dtype=np.float32, copy=False) / 255.0
        )  # [0,255] => [0,1]
    ...
    def _load_normals(self, index):
        ...
        labels = sio.loadmat(
            os.path.join(self.edge_gt_dir, self.im_ids[index] + ".mat")
        )
    ...
    def _check_preprocess_parts(self):
        ...
        if not os.path.isfile(_obj_list_file):
            ...
            return list(np.sort([str(x) for x in self.part_obj_dict.keys()])) == list(
                np.sort(self.im_ids)
            )
    ...
    def _preprocess_parts(self):
        ...
        with open(self.parts_file, "w") as outfile:
            ...
            for i in range(1, len(self.im_ids)):
                outfile.write(
                    ',\n\t"{:s}": {:s}'.format(
                        self.im_ids[i], json.dumps(self.part_obj_dict[self.im_ids[i]])
                    )
                )
            outfile.write("\n}\n")
