...
class NYUD(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        root=MyPath.db_root_dir("NYUDv2"),
        train=True,
        tasks=None,
        transform=None,
        dataidxs=None,
    ...
    def _load_edge(self, index):
        _edge = (
            np.array(Image.open(self.edges[index]), dtype=np.float32, copy=False)
            / 255.0
        )
        return _edge
    ...
    def _load_semseg(self, index):
        _semseg = (
            np.array(Image.open(self.semsegs[index]), dtype=np.float32, copy=False) - 1
        )
